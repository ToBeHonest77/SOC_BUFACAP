...
# Create a custom boxplot with minimal design, displaying quantile lines, median and label
ggplot(data, aes(x = Category, y = Price)) +
  stat_summary(fun.min = function(x) quantile(x, 0.25),
               fun.max = function(x) quantile(x, 0.75),
               geom = "errorbar", 
  ...
  stat_summary(fun = median, 
               ...
               shape = 95, 
               size = 6, 
               colour = "#c2c852") +  # Show median as a horizontal line
  stat_summary(fun = median, 
               geom = "text", 
               aes(label = round(..y.., 1)),  # Label with rounded median value
               vjust = -1.5, 
               size = 4, 
               colour = "black") +  # Position and size the text label
  coord_cartesian(ylim = c(0, 35)) +  # Set y-axis limits
  ...
  ggtitle("Median Carbon Credit Prices Q1 2024 (USD/tCO2e)")
...
#   ggtitle("Carbon Credit Prices Q1 2024 (USD/tCO2e)")
# 
# # Create a custom plot with only the median displayed and labeled
# ggplot(data, aes(x = Category, y = Price)) +
#   stat_summary(fun = median, 
#                geom = "point", 
#                shape = 21, 
#                size = 5, 
#                fill = "blue", 
#                colour = "black") +  # Show median as a filled circle
#   stat_summary(fun = median, 
#                geom = "text", 
#                aes(label = round(..y.., 1)),  # Label with rounded median value
#                vjust = -1.5, 
